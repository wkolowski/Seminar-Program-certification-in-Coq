...
Lemma VeryTrue_inhabited : VeryTrue.
(* begin hide *)
Proof.
  now constructor.
Qed.
...
Lemma bool_not_contractible :
  exists x y : bool,
    x <> y.
(* begin hide *)
Proof.
  now exists true, false.
Qed.
(* end hide *)
...
Lemma VeryTrue_not_contractible :
  exists x y : VeryTrue,
    x <> y.
Proof.
  exists proof, other_proof.
  inversion 1.
Abort.
...
Proof.
  intros Heq.
  assert (Hiff : forall P : Type -> Prop, P unit <-> P bool) by (now intros; rewrite Heq).
  specialize (Hiff (fun T => exists f : unit -> T, Bijective f)); cbn in Hiff.
  apply no_bij_unit_bool.
  rewrite <- Hiff.
  now exists (@id unit), (@id unit).
Qed.
(* end hide *)
...
Lemma bij_VeryTrue_True :
  exists f : VeryTrue -> True,
    Bijective f.
(* begin hide *)
Proof.
  exists (fun _ => I); red.
  exists (fun _ => proof).
  now intuition (apply proof_irrelevance).
Qed.
(* end hide *)
...
Lemma VeryTrue_is_True :
  VeryTrue = True.
(* begin hide *)
Proof.
  apply prop_ext.
  now apply VeryTrue_True.
Qed.
(* end hide *)
...
Lemma TheTruest_contractible :
  forall x y : TheTruest,
    x = y.
(* begin hide *)
Proof.
  now apply proof_irrelevance.
...
Proof.
  ...
  assert (H1 := H' 1).
  assert (H2 := H' 2).
  now destruct (g 0), (g 1), (g 2); congruence.
Qed.
...
Proof.
  intros Heq.
  assert (Hiff : forall P : Type -> Prop, P bool <-> P nat) by (now intros; rewrite Heq).
  specialize (Hiff (fun T => exists f : bool -> T, Bijective f)); cbn in Hiff.
  apply no_bij_bool_nat.
  rewrite <- Hiff.
  now exists (@id bool), (@id bool).
Qed.
...
Proof.
  apply nat_ind.
Restart.
  apply (nat_ind (fun a : nat => forall b c : nat, a + (b + c) = (a + b) + c)).
  - cbn.
    easy.
  - cbn.
    intros.
...
Lemma P_char :
  forall n : nat,
    P n <-> exists k : nat, n = 2 + 3 * k.
(* begin hide *)
Proof.
  split.
  - induction 1.
    + now exists 0; cbn.
    + destruct IHP as [k IH].
      exists (S k).
      now rewrite IH; lia.
  - intros [k ->].
    induction k as [| k'].
    + cbn.
      now constructor.
    + replace (2 + 3 * S k') with (S (S (S (2 + 3 * k')))) by lia.
      now constructor.
Qed.
(* end hide *)
...
Lemma P_char :
  forall n : nat,
    P n <-> exists k l : nat, n = 1 + 2 * k + 3 * l.
(* begin hide *)
Proof.
  split.
  - induction 1.
  ...
  - intros (k & l & ->).
    ...
    + induction l as [| l'].
      * now cbn; constructor.
      * replace (P _) with (P (3 + 1 + 3 * l')) by (now f_equal; lia).
        now apply c2; cbn in *.
    + replace (P _) with (P (2 + 1 + 2 * k' + 3 * l)) by (now f_equal; lia).
      now constructor.
Qed.
(* end hide *)
...
Proof.
  ...
  - now induction 1; [| lia].
  - intros ->.
    induction n as [| n']; [now cbn; constructor |].
    replace (1 + 2 * _) with (2 + 1 + 2 * n') by lia.
    now constructor.
Qed.
(* end hide *)
...
Proof.
  ...
  - induction 1; [easy |].
    rewrite IHP; cbn.
    rewrite (Nat.mul_comm _ (S n)); cbn.
    now lia.
  - intros ->.
    induction n as [| n']; [now cbn; constructor |].
    replace (S n' * S n') with (n' * n' + 2 * n' + 1); [now constructor |].
    cbn; rewrite (Nat.mul_comm _ (S n')); cbn.
    now lia.
Qed.
(* end hide *)
...
Proof.
  ...
  - now induction 1; [| lia].
  - intros ->.
    induction n as [| n']; cbn; [now constructor |].
    rewrite <- !plus_n_Sm.
    now constructor.
Qed.
(* end hide *)
...
Proof.
  split.
  - now induction 1; cbn; lia.
  - now intros ->; induction n as [| n']; cbn; constructor.
Qed.
(* end hide *)
...
Lemma P_char :
  forall a b c : nat,
    P a b c <-> b = fib a /\ c = fib (S a).
(* begin hide *)
Proof.
  split.
  - induction 1; [now cbn |].
    now destruct IHP as [-> ->]; split; cbn; lia.
  - intros [-> ->].
    induction a as [| a']; cbn in *; [now constructor |].
...
Lemma P_char :
  forall a b c : nat,
    P a b c <-> b = 1 + 2 * a /\ c = a * a.
(* begin hide *)
Proof.
  split.
  - induction 1; [now cbn |].
    destruct IHP as [-> ->].
    split; cbn in *; [now lia |].
    now rewrite (Nat.mul_comm _ (S a)); lia.
  - intros [-> ->].
    induction a as [| a']; cbn; [now constructor |].
    rewrite (Nat.mul_comm _ (S _)).
    replace (P _ _ _) with (P (1 + a') (2 + 1 + 2 * a') ((1 + 2 * a') + (a' * a'))) by (now f_equal; cbn; lia).
    now constructor.
Qed.
(* end hide *)
...
Proof.
  ...
  now induction n as [| n']; cbn; intros; constructor.
Qed.
(* end hide *)
...
Proof.
  intros Heq.
  cut (forall P : (nat -> Prop) -> Prop, P T <-> P T'); [| now intros; rewrite Heq].
  intros Hiff.
  assert (H1 : ~ T 1) by now inversion 1.
  apply H1.
  rewrite (Hiff (fun T => T 1)).
  now constructor.
Qed.
(* end hide *)
...
(* begin hide *)
Proof.
  now split; induction 1; cbn; intros; constructor.
Qed.
(* end hide *)
...
(* begin hide *)
Proof.
  now induction 1; cbn; intros; constructor.
Qed.
(* end hide *)
...
Proof.
  - destruct 1.
    + now exists 0.
    + destruct (odd'_2n_1' _ H) as [k ->].
      exists (S k).
      now lia.
  - destruct 1.
    destruct (even'_2n' _ H) as [k ->].
    exists k.
    now lia.
Qed.
...
(**
  ...
  theorem statement). This is because using the command [Lemma] in the
...
Proof.
  now destruct b; cbn.
Restart.
  now destruct b using bool_ind; cbn.
Restart.
  now destruct b using bool_ind'; cbn.
Restart.
  now destruct b using bool_ind''; cbn.
...
Proof.
  ...
  - exact H0.
  - exact H1.
  - apply HSS, nat_ind_2'; assumption.
Defined.
...
(**
  The function [Nat.div2], which performs integer division by two.
  Here is its definition:
*)

Eval compute in Nat.div2.
(* ===>
  = fix Nat.div2 (n : nat) : nat :=
      match n with
      | 0 => 0
      | 1 => 0
      | S (S n') => S (div2 n')
...
Proof.
  ...
  destruct n'; cbn in *; [easy |].
Restart.
  induction n as [| | n'] using nat_ind_2; cbn; intros; [easy.. |].
  destruct n' as [| [| n'']]; cbn in *; [now auto.. |].
  unfold lt in *.
  now apply le_n_S, le_S, IHn', le_n_S, le_0_n.
Qed.
...
Proof.
  intros.
  functional induction @swap_blocks A l; cbn; [easy | easy |].
  now rewrite IHl0.
Qed.
...
Proof.
  intros.
  cut (forall l : list A, P (rev l)); intros.
...
Proof.
  ...
  - now destruct l.
  - destruct l as [| h t]; cbn in *; [easy |].
    rewrite IHn'; [easy |].
    now apply le_S_n.
Restart.
  intros.
  functional induction (@take A n l); cbn in *.
  - now destruct _x.
  - easy.
  - rewrite IHl0; [easy |].
    now apply le_S_n.
...
(* begin hide *)
Proof.
  now induction n as [| n']; destruct l as [| h t];
    cbn; try easy; intros; f_equal; apply IHn', le_S_n.
Restart.
  intros.
  functional induction @take A n l; cbn in *;
    rewrite ?IHl0; try easy.
  - now destruct _x.
  - now apply le_S_n.
Qed.
(* end hide *)
...
Proof.
  ...
  - now apply le_0_n.
  - now apply le_n_S, IHn'.
Restart.
  intros.
  functional induction @take A n l; cbn; [easy | |].
  - now apply le_0_n.
  - now apply le_n_S.
Qed.
(* end hide *)
...
Proof.
  intros.
  case_eq (f true); case_eq (f false); intros Hf Ht.
  - right; right; left.
    now extensionality x; destruct x.
  - left.
    now extensionality x; destruct x.
  - right; left.
    now extensionality x; destruct x.
...
Proof.
  intros.
  now destruct (bool_fun_char f) as [-> | [-> | [-> | ->]]].
Qed.
...
Lemma div2_rel_SS :
  forall n m : nat,
    div2_rel n m -> div2_rel (S (S n)) (S m).
(* begin hide *)
Proof.
  induction 1.
  - replace (S (S (2 * n))) with (2 * (S n)) by lia.
    now constructor.
  - replace (div2_rel _ _) with (div2_rel (2 * (S n) + 1) (S n)) by (now cbn; f_equal; lia).
    now constructor.
Qed.
(* end hide *)
...
Lemma div2_rel_correct :
  forall n m : nat,
    Nat.div2 n = m -> div2_rel n m.
(* begin hide *)
Proof.
  induction n using nat_ind_2; cbn; intros; subst.
  - change 0 with (2 * 0) at 1.
    now constructor.
  - change 1 with (2 * 0 + 1) at 1.
    now constructor.
  - now apply div2_rel_SS, IHn.
Qed.
(* end hide *)
...
Lemma div2_rel_complete :
  forall n m : nat,
    div2_rel n m -> Nat.div2 n = m.
(* begin hide *)
Proof.
  induction 1.
  - induction n using nat_ind_2; cbn in *; [easy | easy |].
    rewrite Nat.add_comm; cbn.
    do 2 f_equal.
    rewrite <- IHn at 3; f_equal.
    now lia.
  - rewrite Nat.add_comm.
    induction n using nat_ind_2; cbn in *; [easy | easy |].
    rewrite <- plus_n_O, Nat.add_comm in *; cbn in *.
    now rewrite IHn.
Qed.
(* end hide *)
...
Lemma Zfact_nonneg :
  forall n : nat,
    Zfact (Z.of_nat n) (Z.of_nat (fact n)).
(* begin hide *)
Proof.
  induction n as [| n']; [now cbn; constructor |].
  change (fact (S n')) with (S n' * fact n')%nat.
  rewrite Nat2Z.inj_mul, Nat2Z.inj_succ.
  constructor.
  now apply IHn'.
Qed.
(* end hide *)
...
Lemma collatz_42 :
  collatz 42 1.
Proof.
  apply collatz_even; cbn; [now repeat constructor |].
  apply collatz_odd; cbn; [now repeat constructor |].
  apply collatz_even; cbn; [now repeat constructor |].
  apply collatz_even; cbn; [now repeat constructor |].
  apply collatz_even; cbn; [now repeat constructor |].
  apply collatz_even; cbn; [now repeat constructor |].
...
(* ===>
  div2_ind :
    forall P : nat -> nat -> Prop,
      ...
      (forall n n' : nat, n = S (S n') -> P n' (div2 n') -> P (S (S n')) (S (div2 n'))) ->
...
Lemma R_div2'_correct :
  forall n m : nat,
    Nat.div2 n = m -> R_div2' n m.
Proof.
  now induction n as [| | n'] using nat_ind_2; cbn; intros; subst; constructor; auto.
Qed.

Lemma R_div2'_complete :
  forall n m : nat,
    R_div2' n m -> Nat.div2 n = m.
Proof.
...
(* ===>
  R_div2_correct :
    forall n _res : nat, _res = Nat.div2 n -> R_div2 n _res
*)

Check R_div2_complete.
(* ===>
  R_div2_complete :
    forall n _res : nat, R_div2 n _res -> _res = Nat.div2 n
*)
...
Proof.
  intros.
  apply R_div2'_ind; [easy | easy | |].
  - intros.
    apply R_div2'_complete in H2 as <-.
    now apply H1.
  - now apply R_div2'_correct.
Qed.
...
Proof.
  intros n.
  apply div2_ind'; [now lia | now lia |].
  intros [| [| m']] H1 H2; cbn in *; [now lia | now lia |].
  rewrite <- Nat.succ_lt_mono.
  now apply Nat.lt_trans with (S (S m')); lia.
Restart.
  intros n.
  functional induction @div2 n; cbn; [now lia | now lia |].
  intros H.
  destruct n' as [| [| n]]; cbn in *; [now lia | now lia |].
  rewrite <- Nat.succ_lt_mono.
  now apply Nat.lt_trans with (S (S n)); lia.
Qed.
...
Proof.
  ...
  now apply R_div2l'_correct.
Qed.

Lemma R_div2l'_complete :
  forall (A : Type) (l l' : list A),
    R_div2l' l l' -> div2l l = l'.
Proof.
  now induction 1; cbn; subst.
Qed.
...
Proof.
  ...
  - now apply H0.
  - now apply H1.
  - now apply H2, div2l_ind'.
Qed.

Lemma div2l_equation' :
  forall (A : Type) (l : list A),
    div2l l
      =
    match l with
...
Proof.
  intros A l.
  apply div2l_ind'; cbn; intros; [easy | now auto |].
  destruct l0 as [| h1 [| h2 t]]; cbn in *; [now auto.. |].
  rewrite <- Nat.succ_lt_mono.
  apply Nat.lt_trans with (S (S (length t))); [| now auto].
  now apply H.
Qed.
(* end hide *)
...
Proof.
  induction m as [| m']; cbn; intros; [easy |].
  rewrite IHm'.
Abort.
...
(* begin hide *)
Proof.
  induction c as [| c']; cbn; [easy |].
  rewrite plus'_S; cbn.
  now rewrite !plus'_S, IHc'.
Qed.
(* end hide *)
...
Proof.
  ...
  induction c as [| c']; cbn; intros; [easy |].
  rewrite !IHc'; cbn.
  now rewrite plus'_S.
Qed.
(* end hide *)
...
Lemma rev_acc_spec_strong :
  forall (A : Type) (l acc : list A),
    rev_acc l acc = rev l ++ acc.
Proof.
  induction l as [| h t]; cbn; intros; [easy |].
  now rewrite IHt, <- app_assoc; cbn.
Qed.
...
Lemma rev_acc_spec_weak :
  forall (A : Type) (l : list A),
    rev_acc l [] = rev l.
Proof.
  now intros; rewrite rev_acc_spec_strong, app_nil_r.
Qed.
...
Proof.
  unfold app'.
  intros.
  now rewrite !revapp_spec, app_nil_r, rev_involutive.
Qed.
(* end hide *)
...
Restart.
  intros.
  remember (While b i) as w.
  now induction H1; intros; inversion Heqw; subst; eauto.
Qed.
...
Ltac ind' H :=
match type of H with
| ?T => replace_nonvars T; induction H; subst; try congruence; invs; clean_eqs; eauto
end.
